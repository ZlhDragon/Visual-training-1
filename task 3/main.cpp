...
int main() 
{
    ...
    if (points.size() == num_points) 
    {
        ...
        // 定义车牌的颜色范围 (HSV)
        Scalar lower_plate(100, 75, 0);
        Scalar upper_plate(150, 255, 155);
        ...
        // adaptiveThreshold(gray_plate, adaptive_binary_plate, 255, ADAPTIVE_THRESH_GAUSSIAN_C, THRESH_BINARY_INV, 11, 2);

        //  // 腐蚀操作
        // Mat kernel = getStructuringElement(MORPH_ELLIPSE, Size(5, 5)); // 创建椭圆形结构元素
        // Mat eroded_plate;
        // erode(binary_plate, eroded_plate, kernel); // 应用腐蚀操作
        ...
        // imshow("Eroded Plate", eroded_plate); // 显示腐蚀后的图像
        waitKey(0);
    } else 
    {
